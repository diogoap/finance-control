...
function incomesModalController($scope, $uibModal, $uibModalInstance, uiGridConstants, Utils, Incomes, Categories, Accounts, Currencies, incomeId, action) {
	$scope.loading = true;
	$scope.income = {};
	$scope.Utils = Utils;
	$scope.alerts = [];
	...
	Currencies.get(filter)
		.then(function onSucess(response) {
			...
			if (action == 'new') {
				$scope.income.currency_id = Utils.getDefaultCurrencyId(response.data);;
			}
	...
	$scope.updateIncomeTotal = function () {
		...
		$scope.income.dueDate = Utils.getDateDst($scope.income.dueDate);
		...
		if (($scope.income.detail != undefined) && ($scope.income.detail.length > 0)) {
			...
			$scope.income.currency_id = '';
			$scope.income._currency = null;
